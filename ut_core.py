...
class UTCoreMenuNavigator:
    ...
    #def select_menu(self, list query, list input=None):
        #test_index = self.UT.find_index_in_output(group_output, menu["name"])
        #test_output = self.session.write(str(test_index))

       # sub_menu = self.find_key(menu, "sub_menu")
       # if (sub_menu):
       #     for key, value in sub_menu.items():
       #         menu_index = self.UT.find_index_in_output(test_output, value)
       #         menu_output = self.session.write(str(menu_index))
       #         return menu_output
       # return ''
